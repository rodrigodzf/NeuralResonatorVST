...
template <typename T>
inline void ParallelLerp<T>::setup(
    const T* values,
...
{
    if (nValues != m_interpolators.size()) return false;
    for (unsigned int i = 0; i < nValues; i++)
    {
...
template <typename T>
inline bool
    ParallelLerp<T>::setTargets(const T* targets, unsigned int nTargets)
{
    if (nTargets != m_interpolators.size()) return false;
...
inline bool ParallelLerp<T>::isFinished()
{
    for (unsigned int i = 0; i < m_interpolators.size(); i++)
    {
        if (!m_interpolators[i].isFinished()) return false;
    }
    return true;
}
...
{
    if (index >= m_interpolators.size()) return T();
...
{
    if (index >= m_interpolators.size()) return false;
...
{
    if (m_interpolators.size() == 0) return 0;
