...
// src
import { MutablePoint } from './mutable_point'
...
export const Drum: React.FC<{
	...
	*/
	...
	useEffect(() => updateStrike(strike), [strike])

	return (
		<div className='drum'>
			<svg version='1.1' x='0px' y='0px' viewBox='0 0 100 100' xmlSpace='preserve'>
				<polygon
					points={_polygon.map((p: Point) => `${p.x * 100},${p.y * 100}`).join(' ')}
				/>
			</svg>
			...
			<MutablePoint
				className='strike'
				point={_strike}
				onDrag={(p: Point, _) => onStrikeChange(p)}
			/>
		</div>
	)
}
