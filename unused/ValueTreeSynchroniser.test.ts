...
import { InputStream } from '../ui/src/components/valueTree/InputStream'
import { ValueTree } from '../ui/src/components/valueTree/ValueTree'
import { applyChange } from '../ui/src/components/valueTree/ValueTreeSynchroniser'

const getTree = () => {
	const file = readFileSync(join(TEST_ASSETS_PATH, 'valueTree', 'valueTreeFull.bin'))
...
const readChange = (name: string) => {
	const data = new Uint8Array(readFileSync(join(TEST_ASSETS_PATH, 'syncMessages', `${name}.bin`)))
	return data
}
